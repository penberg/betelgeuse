...
io_test! {
    // Ops are not ordered relative to each other; callers that need a stat
    // to observe a write must wait for the write completion first. A stat
    // in flight concurrently with a write sees the size before or after
    // the write, never anything else.
    fn concurrent_pwrite_and_stat_both_complete(io_loop) -> io::Result<()> {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("concurrent-stat.bin");
        let file = io_loop.io().open(&path, rw_create_truncate())?;
        ...
        let size = stat_c.take_result().unwrap()?;
        assert!(size == 0 || size == 3, "stat saw torn size {size}");
        Ok(())
    }
}
